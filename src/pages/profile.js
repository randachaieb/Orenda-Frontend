...
function Profile({ data, newObject }) {
  ...
  const fetchPosts = async () => {
    axios
      .get(
        `http://localhost:5000/api/v1/user/posts/${id}?limit=${limit}&page=${page}`,
        {
          headers: {
  ...
  useEffect(async () => {
    axios
      .get(`http://localhost:5000/api/v1/user/profile/${id}`, {
        headers: {
          "x-auth-token": localStorage.getItem("token"),
        },
      })
  ...
  const handleFollow = async () => {
    ...
    try {
      const res = await axios.post(
        `http://localhost:5000/api/v1/user/follow/${profile._id}`,
        null,
        {
          headers: {
            "x-auth-token": localStorage.getItem("token"),
  ...
  const handleUnfollow = async () => {
    ...
    try {
      const res = await axios.post(
        `http://localhost:5000/api/v1/user/unfollow/${profile._id}`,
        null,
        {
          headers: {
            "x-auth-token": localStorage.getItem("token"),
      ...
      if (res.status === 200) {
        console.log("success");
        const updatedFollowing = authContext.user.following.filter(
