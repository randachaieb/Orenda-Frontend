...
class PopupForm extends React.Component {
  ...
  handlechangeOffer = (event) => {
    axios
      .get(
        "http://localhost:5000/api/v1/categories/offerCategoriesSub/" + event
      )
      .then((res) => {
  ...
  handlechangecategory = (event) => {
    axios
      .get(
        "http://localhost:5000/api/v1/categories/PlaceCategoriesSub/" + event
      )
      .then((res) => {
  ...
  formSubmit(event) {
    ...
    axios
      .post("http://localhost:5000/api/v1/card", params, {
        headers: {
          ...
          {this.state.subcategoryOffers ? (
            <div>
              <Select
                ...
                placeholder="Please select"
                style={{ width: "100%" }}
                onChange={this.handlechangesuboffer}
                required
              >
